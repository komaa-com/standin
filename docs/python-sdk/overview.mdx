...
`standin-sdk` is the worker side of a StandIn call. StandIn handles the Microsoft Teams side and talks to your worker over one authenticated WebSocket per call. This package is that socket's other end.
...
**The Microsoft side is StandIn's.** StandIn handles it. Your worker never holds a Bot Framework credential and never talks to Microsoft.
