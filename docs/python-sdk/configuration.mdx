...
<Warning>
**Python and TypeScript do not treat a blank `STANDIN_CHAT_SECRET` or a bad `STANDIN_PORT` the same way.** Unset `STANDIN_CHAT_SECRET` rather than exporting it empty, and never ship a non-numeric `STANDIN_PORT`. Details are under [Three things that cost an afternoon](#three-things-that-cost-an-afternoon). The TypeScript twin is [Configuration](/typescript-sdk/configuration).
</Warning>
...
| `STANDIN_STATE_DIR` | Where a parked outbound message lives until the person answers, and the default parent of unfinished meeting recaps. Default `~/.standin/state`, created owner-only. Deliberately not a temp directory. | [Reaching people](/python-sdk/reaching-people), [Meeting recap](/python-sdk/minutes#from-the-openclaw-and-hermes-plugins) |
| `STANDIN_RECAP_DIR` | Where unfinished meeting recaps sit until they post, expire (24 hours), or hit the retry limit (8). Restart-recoverable local spool. Default `$STANDIN_STATE_DIR/recap`. Best-effort unless this directory is on storage that survives a process or container restart: the default is local process disk, which is ephemeral in most containers. | [Meeting recap](/python-sdk/minutes#from-the-openclaw-and-hermes-plugins) |
