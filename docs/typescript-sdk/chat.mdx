...
it opens at all you are managed. The [sandbox](/tiers#the-sandbox) has no chat lane. A meeting recap
that only posts uses `listenOnly: true` so it never answers.

## Meeting recap

`listenOnly: true` is how a recap lane posts without answering. The host must supply that outbound chat
lane (`chat.start()`) and a summarization callback (`postMinutes` or the plugin consult). Hang-up
does not await the post.
...
| `listenOnly` | `false` | Take messages without answering them. `chats` is still fed, `respond` is never called, and nothing is sent back, not even the typing indicator. For a lane that only posts, such as a meeting recap, when something else already answers this connection's chat. |
