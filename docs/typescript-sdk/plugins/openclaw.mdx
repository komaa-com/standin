...
OpenClaw's external HTTP surface is text-only, so a worker that only called that HTTP surface could only use OpenClaw as a
...
| `sessionScope` | Memory of the agent session that writes the minutes: `per-call` (default), `per-thread`, or `per-aad`. The voice session is always per call. |
| `meetingRecap` | After the call ends, post minutes into the Microsoft Teams chat. Off unless exactly `true`. Needs the StandIn chat lane ([Tiers](/tiers#the-standin-managed-bot)) and a summarization consult. Best-effort: hang-up does not await the post. |
...
`allowFrom` takes the caller's **AAD object id**, not their email. Copy it from
`session.start.caller.aadId` on an inbound call (a refused call still logs it). Emails never match.
It matches on exact id, case-insensitive, or on digits-only for a phone number. The id match is what
lets a Microsoft Teams caller be allowlisted at all: their id is an AAD object id, and phone
normalization would reduce it to the empty string and match nothing.
...
- **Chat and messages** belong to the SDK's own [chat lane](/typescript-sdk/chat), which is a
  separate surface with its own page. When `meetingRecap` is on, this plugin opens that lane
  listen-only so it can post minutes without answering chat.
- **Meeting recap** is this plugin's `meetingRecap` switch. Off by default. See
  [Meeting recap](#meeting-recap) below, and the library it calls in
  [Meeting recap](/typescript-sdk/minutes).
- **Vision, avatar expression and visemes** are on the StandIn call wire and in the SDK core, and
  this plugin does not wire them into the OpenClaw realtime session. That is a boundary of this
  plugin, not of the call. Reach for `VisionTools` and `CallTools` from a handler of your own:
  [Vision and the avatar](/typescript-sdk/vision) and [The avatar](/typescript-sdk/avatar).
...
Off until you set `meetingRecap: true`. The plugin then:

1. Collects the call transcript.
2. Writes a restart-recoverable local spool before hang-up returns (`STANDIN_RECAP_DIR`, else `STANDIN_STATE_DIR/recap`, else `~/.standin/state/recap`). A process restart drains what was left if that directory survives the process. Treat recap as best-effort unless that directory is on storage that survives a process or container restart. The file stays until posted, expired (24 hours), or the retry limit (8).
3. Opens its own **listen-only** [chat lane](/typescript-sdk/chat) and posts minutes on it. That lane never answers chat, even if something else on the host already does.

The host must provide an outbound chat lane and a summarization consult. Hang-up does not wait for the consult. A recap that cannot open the chat lane is logged and not posted. Meeting recaps are best-effort. They may be lost if the worker exits during processing.
...
`sessionScope` keys the **consult that writes the minutes**, not the voice session. Voice is always
